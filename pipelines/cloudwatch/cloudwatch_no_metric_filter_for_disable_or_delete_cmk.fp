locals {
  cloudwatch_no_metric_filter_for_disable_or_delete_cmk_query = <<-EOQ
    with filter_data as (
      select
        trail.account_id,
...
trigger "query" "detect_and_correct_cloudwatch_no_metric_filter_for_disable_or_delete_cmk" {
  title         = "Detect & correct CloudWatch log groups without Disable or Delete CMK metric filter"
  ...
  // documentation = file("./cloudwatch/docs/detect_and_correct_cloudwatch_no_metric_filter_for_disable_or_delete_cmk_trigger.md")
  tags          = merge(local.cloudwatch_common_tags, { class = "unused" })

  enabled  = var.cloudwatch_no_metric_filter_for_disable_or_delete_cmk_trigger_enabled
  schedule = var.cloudwatch_no_metric_filter_for_disable_or_delete_cmk_trigger_schedule
  database = var.database
  sql      = local.cloudwatch_no_metric_filter_for_disable_or_delete_cmk_query

  capture "insert" {
    pipeline = pipeline.correct_cloudwatch_no_metric_filter_for_disable_or_delete_cmk
    args = {
...
pipeline "detect_and_correct_cloudwatch_no_metric_filter_for_disable_or_delete_cmk" {
  title         = "Detect & correct CloudWatch log groups without Disable or Delete CMK metric filter"
  description   = "Detects CloudWatch log groups that do not have a metric filter for Disable or Delete CMK and runs your chosen action."
  // documentation = file("./cloudwatch/docs/detect_and_correct_cloudwatch_no_metric_filter_for_disable_or_delete_cmk.md")
  ...
  param "default_action" {
    ...
    default     = var.cloudwatch_no_metric_filter_for_disable_or_delete_cmk_default_action
  }

  param "enabled_actions" {
    type        = list(string)
    description = local.description_enabled_actions
    default     = var.cloudwatch_no_metric_filter_for_disable_or_delete_cmk_default_actions
  }
...
pipeline "correct_cloudwatch_no_metric_filter_for_disable_or_delete_cmk" {
  title         = "Correct CloudWatch log groups without Disable or Delete CMK metric filter"
  ...
  // documentation = file("./cloudwatch/docs/correct_cloudwatch_no_metric_filter_for_disable_or_delete_cmk.md")
  tags          = merge(local.cloudwatch_common_tags, { class = "unused" })
  ...
  step "message" "notify_detection_count" {
    ...
    text     = "Detected ${length(param.items)} CloudWatch log groups without Disable or Delete CMK metric filter."
  }
  ...
  step "pipeline" "correct_item" {
    ...
    pipeline        = pipeline.correct_one_cloudwatch_no_metric_filter_for_disable_or_delete_cmk
    args = {
...
pipeline "correct_one_cloudwatch_no_metric_filter_for_disable_or_delete_cmk" {
  title         = "Correct one CloudWatch log group without Disable or Delete CMK metric filter"
  description   = "Runs corrective action on a CloudWatch log group without Disable or Delete CMK metric filter."
  // documentation = file("./cloudwatch/docs/correct_one_cloudwatch_no_metric_filter_for_disable_or_delete_cmk.md")
  tags          = merge(local.cloudwatch_common_tags, { class = "unused" })
  ...
  step "pipeline" "respond" {
    ...
    args = {
      ...
      notification_level = param.notification_level
      approvers          = param.approvers
      detect_msg         = "Detected CloudWatch log group without Disable or Delete CMK metric filter for account ${param.title}."
      default_action     = param.default_action
      enabled_actions    = param.enabled_actions
      actions = {
        "skip" = {
          ...
          pipeline_args = {
            ...
            text     = "Skipped CloudWatch log group without Disable or Delete CMK metric filter for account ${param.title}."
          }
          success_msg = ""
          error_msg   = ""
        },
        "enable_disable_or_delete_cmk_metric_filter" = {
          ...
          pipeline_args = {
            cred             = param.cred
            region           = "us-east-1"
            log_group_name   = "log_group_name_40"
            filter_name      = "DisableDeleteCMKMetric"
            role_name        = "DisableDeleteCMKMetricRole"
            trail_name       = "DisableDeleteCMKCMetricTrail"
            s3_bucket_name   = "disabledeletecmkmetrics3bucket"
            metric_name      = "DisableDeleteCMKMetrics"
            metric_namespace = "CISBenchmark"
            queue_name       = "flowpipeDisableDeleteCMK"
            metric_value     = "1"
            filter_pattern   = "{($.eventSource = kms.amazonaws.com) && (($.eventName=DisableKey)||($.eventName=ScheduleKeyDeletion)) }"
            sns_topic_name = "disable_delete_cmk_metric_topic"
            protocol       = "SQS"
            alarm_name     = "disable_delete_cmk_alarm"
            ...
            "Statement": [
              {
                ...
                "Resource": "arn:aws:s3:::disabledeletecmkmetrics3bucket"
              },
              {
                ...
                "Action": "s3:PutObject",
                "Resource": "arn:aws:s3:::disabledeletecmkmetrics3bucket/AWSLogs/533793682495/*",
                "Condition": {
                  "StringEquals": {
...
variable "cloudwatch_no_metric_filter_for_disable_or_delete_cmk_trigger_enabled" {
  type        = bool
  default     = false
  description = "If true, the trigger is enabled."
}

variable "cloudwatch_no_metric_filter_for_disable_or_delete_cmk_trigger_schedule" {
  type        = string
  default     = "15m"
  description = "The schedule on which to run the trigger if enabled."
}

variable "cloudwatch_no_metric_filter_for_disable_or_delete_cmk_default_action" {
  type        = string
  description = "The default action to use for the detected item, used if no input is provided."
  default     = "notify"
}

variable "cloudwatch_no_metric_filter_for_disable_or_delete_cmk_default_actions" {
  type        = list(string)
