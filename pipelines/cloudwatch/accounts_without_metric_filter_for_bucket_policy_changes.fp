locals {
  accounts_without_metric_filter_for_bucket_policy_changes_query = <<-EOQ
    with trails as (
      select
        trail.account_id,
...
variable "accounts_without_metric_filter_for_bucket_policy_changes_trigger_enabled" {
  type        = bool
  default     = false
...
variable "accounts_without_metric_filter_for_bucket_policy_changes_trigger_schedule" {
  type        = string
  default     = "15m"
...
trigger "query" "detect_and_correct_accounts_without_metric_filter_for_bucket_policy_changes" {
  ...
  sql      = local.accounts_without_metric_filter_for_bucket_policy_changes_query

  capture "insert" {
    pipeline = pipeline.correct_accounts_without_metric_filter_for_bucket_policy_changes
    args = {
...
pipeline "correct_accounts_without_metric_filter_for_bucket_policy_changes" {
  ...
  step "message" "notify_detection_count" {
    ...
    notifier = param.notifier
    text     = "Detected ${length(param.items)} account(s) without metric filter for bucket policy changes."
  }
  ...
  step "message" "notify_items" {
    ...
    notifier = param.notifier
    text     = "Detected account ${each.value.title} without metric filter for bucket policy changes."
  }
}
