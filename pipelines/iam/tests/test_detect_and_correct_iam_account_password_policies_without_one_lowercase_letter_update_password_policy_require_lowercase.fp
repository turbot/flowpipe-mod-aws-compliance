pipeline "test_detect_and_correct_iam_account_password_policies_without_one_lowercase_letter_update_password_policy_require_lowercase" {
  ...
  step "pipeline" "disable_password_policy_require_lowercase" {
    depends_on = [step.query.get_password_policy_without_lowercase_letter_requirement]
    if        = length(step.query.get_password_policy_without_lowercase_letter_requirement.rows) == 0 && (step.query.get_password_policy.rows[0].password_policy_account_id) != null
  ...
  step "container" "delete_iam_account_password_policy" {
    if     =(step.query.get_password_policy.rows[0].password_policy_account_id) == null
    depends_on = [step.pipeline.set_password_policy_require_lowercase_to_old_setting]
  ...
  output "test_results" {
    ...
    value = {
      ...
      "disable_password_policy_require_lowercase" = (step.query.get_password_policy.rows[0].password_policy_account_id) != null ? !is_error(step.pipeline.disable_password_policy_require_lowercase) ? "pass" : "fail: ${error_message(step.pipeline.disable_password_policy_require_lowercase)}" : "Not required to disable require lowercase"
