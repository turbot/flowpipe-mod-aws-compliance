locals {
  rds_db_cluster_if_iam_authentication_disabled_query = <<-EOQ
    select
...
trigger "query" "detect_and_correct_rds_db_cluster_if_iam_authentication_disabled" {
  ...
  sql      = local.rds_db_cluster_if_iam_authentication_disabled_query

  capture "insert" {
    pipeline = pipeline.correct_rds_db_cluster_if_iam_authentication_disabled
    args = {
...
pipeline "detect_and_correct_rds_db_cluster_if_iam_authentication_disabled" {
  ...
  param "default_action" {
    ...
    default     = var.rds_db_cluster_if_iam_authentication_disabled_default_action
  }

  param "enabled_actions" {
    type        = list(string)
    description = local.description_enabled_actions
    default     = var.rds_db_cluster_if_iam_authentication_disabled_enabled_actions
  }
...
pipeline "correct_rds_db_cluster_if_iam_authentication_disabled" {
  ...
  step "message" "notify_detection_count" {
    ...
    text     = "Detected ${length(param.items)} RDS DB clusters if IAM authentication is disabled."
  }
...
pipeline "correct_one_rds_db_cluster_if_iam_authentication_disabled" {
  ...
  param "iam_database_authentication_enabled" {
    type        = bool
    description = "Enables the IAM database authentication property of a DB cluster"
  }
  ...
  step "pipeline" "respond" {
    ...
    args = {
      ...
      notification_level = param.notification_level
      approvers          = param.approvers
      detect_msg         = "Detected RDS DB cluster with IAM authentication disabled ${param.title}."
      default_action     = param.default_action
      enabled_actions    = param.enabled_actions
      actions = {
        "skip" = {
          ...
          pipeline_args = {
            ...
            text     = "Skipped RDS DB cluster ${param.title} with IAM authentication disabled."
          }
          success_msg = ""
          error_msg   = ""
        },
...
variable "rds_db_cluster_if_iam_authentication_disabled_trigger_enabled" {
  type        = bool
  default     = false
  description = "If true, the trigger is enabled."
}

variable "rds_db_cluster_if_iam_authentication_disabled_trigger_schedule" {
  type        = string
  default     = "15m"
  description = "If the trigger is enabled, run it on this schedule."
}

variable "rds_db_cluster_if_iam_authentication_disabled_default_action" {
  type        = string
  description = "The default action to use when there are no approvers."
  default     = "notify"
}

variable "rds_db_cluster_if_iam_authentication_disabled_enabled_actions" {
  type        = list(string)
