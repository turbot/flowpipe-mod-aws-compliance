locals {
  vpc_security_groups_allowing_ingress_to_port_445_query = <<-EOQ
    ...
    select
      ...
      ingress_cifs_rules.security_group_rule_id as security_group_rule_id,
      sg.region as region,
      ingress_cifs_rules.ip_protocol as ip_protocol,
      ingress_cifs_rules.from_port as from_port,
      ingress_cifs_rules.to_port as to_port,
      ingress_cifs_rules.cidr_ipv4 as cidr_ipv4,
      ingress_cifs_rules.cidr_ipv6 as cidr_ipv6,
...
variable "vpc_security_groups_allowing_ingress_to_port_445_trigger_schedule" {
  type        = string
  default     = "15m"
...
variable "vpc_security_groups_allowing_ingress_to_port_445_enabled_actions" {
  type        = list(string)
...
trigger "query" "detect_and_correct_vpc_security_groups_allowing_ingress_to_port_445" {
  ...
  sql      = local.vpc_security_groups_allowing_ingress_to_port_445_query

  capture "insert" {
    pipeline = pipeline.correct_vpc_security_groups_allowing_ingress_to_port_445
    args = {
...
pipeline "correct_vpc_security_groups_allowing_ingress_to_port_445" {
  ...
  step "message" "notify_detection_count" {
    ...
    notifier = param.notifier
    text     = "Detected ${length(param.items)} VPC security group rule(s) allowing ingress to port 445 from 0.0.0.0/0 or ::/0."
  }

  step "pipeline" "correct_item" {
    for_each        = { for item in param.items : item.security_group_rule_id => item }
    max_concurrency = var.max_concurrency
    pipeline        = pipeline.correct_one_vpc_security_group_allowing_ingress_to_port_445
    args = {
