locals {
  accounts_without_metric_filter_for_cloudtrail_configuration_query = <<-EOQ
    with trails as (
      select
        trail.account_id,
...
variable "accounts_without_metric_filter_for_cloudtrail_configuration_trigger_enabled" {
  type        = bool
  default     = false
...
variable "accounts_without_metric_filter_for_cloudtrail_configuration_trigger_schedule" {
  type        = string
  default     = "15m"
...
trigger "query" "detect_and_correct_accounts_without_metric_filter_for_cloudtrail_configuration" {
  ...
  sql      = local.accounts_without_metric_filter_for_cloudtrail_configuration_query

  capture "insert" {
    pipeline = pipeline.correct_accounts_without_metric_filter_for_cloudtrail_configuration
    args = {
...
pipeline "correct_accounts_without_metric_filter_for_cloudtrail_configuration" {
  ...
  step "message" "notify_detection_count" {
    ...
    notifier = param.notifier
    text     = "Detected ${length(param.items)} account(s) without metric filter for CloudTrail configuration."
  }
  ...
  step "message" "notify_items" {
    ...
    notifier = param.notifier
    text     = "Detected account ${each.value.title} without metric filter for CloudTrail configuration."
  }
}