// TODO: Can I use a string instead of object?
locals {
  cis_v300_2_control_mapping = {
    #cis_v300_2_1_1 = {pipeline = pipeline.cis_v300_2_1_1}
...
pipeline "cis_v300_2_2_1" {
  ...
  step "pipeline" "run_pipeline" {
    depends_on = [step.message.header]
    pipeline   = pipeline.detect_and_correct_ebs_volumes_with_encryption_at_rest_disabled
...
pipeline "cis_v300_2" {
  ...
  step "pipeline" "cis_v300_2" {
    ...
    loop {
      until = loop.index >= (length(var.cis_v300_2_enabled_pipelines)-1)
    }

    #pipeline = local.cis_v300_2_control_mapping[var.cis_v300_2_enabled_pipelines[loop.index]].pipeline
    pipeline = local.cis_v300_2_control_mapping[var.cis_v300_2_enabled_pipelines[loop.index]]
    args     = {
      database           = param.database
      notifier           = param.notifier
      notification_level = param.notification_level
...
pipeline "cis_v300_2_1" {
  ...
  step "pipeline" "run_pipelines" {
    ...
    loop {
      until = loop.index >= (length(var.cis_v300_2_1_enabled_pipelines)-1)
    }

    pipeline = local.cis_v300_2_1_control_mapping[var.cis_v300_2_1_enabled_pipelines[loop.index]].pipeline
    args     = {
      database           = param.database
      notifier           = param.notifier
      notification_level = param.notification_level
