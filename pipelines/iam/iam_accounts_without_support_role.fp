locals {
  iam_accounts_without_support_role_query = <<-EOQ
...
variable "iam_accounts_without_support_role_trigger_schedule" {
  type        = string
...
variable "iam_accounts_without_support_role_enabled_actions" {
  type        = list(string)
...
variable "iam_accounts_without_support_role_user_arn" {
  type        = string
...
variable "iam_accounts_without_support_role_support_role_name" {
  type        = string
...
trigger "query" "detect_and_correct_iam_accounts_without_support_role" {
  title       = "Detect & correct IAM accounts without support role"
  description = "Detects IAM accounts without support role and then create a new support role."
  tags        = local.iam_common_tags

  enabled  = var.iam_accounts_without_support_role_trigger_enabled
  schedule = var.iam_accounts_without_support_role_trigger_schedule
  database = var.database
  sql      = local.iam_accounts_without_support_role_query

  capture "insert" {
    pipeline = pipeline.correct_iam_accounts_without_support_role
    args = {
...
pipeline "detect_and_correct_iam_accounts_without_support_role" {
  title       = "Detect & correct IAM accounts without support role"
  description = "Detects IAM accounts without support role and then create a new support role."
  tags        = local.iam_common_tags
  ...
  param "user_arn" {
    ...
    default     = var.iam_accounts_without_support_role_user_arn
  }

  param "support_role_name" {
    type        = string
    description = "Specifies the IAM support role that will be created."
    default     = var.iam_accounts_without_support_role_support_role_name
  }
...
pipeline "correct_iam_accounts_without_support_role" {
  title       = "Correct IAM accounts without support role"
  description = "Create a new support role for IAM accounts without support role"
  ...
  step "message" "notify_detection_count" {
    ...
    notifier = param.notifier
    text     = "Detected ${length(param.items)} IAM accounts without support role."
  }

  step "pipeline" "correct_item" {
    for_each        = { for row in param.items : row.title => row }
    max_concurrency = var.max_concurrency
    pipeline        = pipeline.correct_one_iam_accounts_without_support_role
    args = {
...
pipeline "correct_one_iam_accounts_without_support_role" {
