locals {
  s3_buckets_with_mfa_delete_disabled = <<-EOQ
    select
...
trigger "query" "detect_and_correct_s3_buckets_with_mfa_delete_disabled" {
  ...
  description = "Detect S3 buckets with MFA delete disabled and then enable MFA delete."
  ...
  sql      = local.s3_buckets_with_mfa_delete_disabled
...
pipeline "correct_s3_buckets_with_mfa_delete_disabled" {
  ...
  description = "Enable MFA delete for S3 buckets with MFA delete disabled."
  ...
  step "message" "notify_items" {
    ...
    notifier = notifier[param.notifier]
    text     = "Detected S3 bucket ${each.value.title} with MFA deleted disabled."
  }
}
