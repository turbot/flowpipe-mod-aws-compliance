pipeline "test_detect_and_correct_iam_accounts_password_policy_without_one_lowercase_letter_update_password_policy_require_lowercase" {
  title       = "Test IAM accounts password policy without one lowercase letter requirement"
  ...
  step "container" "set_password_policy_require_lowercase" {
  ...
  step "sleep" "sleep_100_seconds" {
    depends_on = [ step.container.set_password_policy_require_lowercase ]
    duration   = "150s"
  }

  step "pipeline" "run_detection" {
    depends_on = [step.sleep.sleep_100_seconds]
    pipeline = pipeline.detect_and_correct_iam_accounts_password_policy_without_one_lowercase_letter
    args = {
      approvers       = []
  ...
  output "test_results" {
    ...
    value = {
      ...
      "set_password_policy_require_lowercase" = !is_error(step.container.set_password_policy_require_lowercase) ? "pass" : "fail: ${error_message(step.container.set_password_policy_require_lowercase)}"
