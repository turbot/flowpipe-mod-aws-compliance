pipeline "test_detect_and_correct_iam_account_password_policies_without_one_symbol_update_password_policy_require_symbols" {
  ...
  step "pipeline" "disable_password_policy_require_symbol" {
    depends_on = [step.query.get_password_policy_without_symbol_requirement]
    if         = length(step.query.get_password_policy_without_symbol_requirement.rows) == 0 && (step.query.get_password_policy.rows[0].password_policy_account_id) != null
  ...
  step "container" "delete_iam_account_password_policy" {
    if         = (step.query.get_password_policy.rows[0].password_policy_account_id) == null
    depends_on = [step.pipeline.set_password_policy_require_symbol_to_old_setting]
  ...
  output "test_results" {
    ...
    value = {
      ...
      "disable_password_policy_require_symbol" = (step.query.get_password_policy.rows[0].password_policy_account_id) != null ? !is_error(step.pipeline.disable_password_policy_require_symbol) ? "pass" : "fail: ${error_message(step.pipeline.disable_password_policy_require_symbol)}" : "Not required to disable policy to require symbol"
