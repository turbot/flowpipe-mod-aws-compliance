locals {
  ...
  rds_db_instance_if_deletion_protection_disabled_query = <<-EOQ
    select
    ...
    from
      aws_rds_db_instance
    where
      (not engine like any(array['aurora%', 'docdb', 'neptune']))
      and not deletion_protection;
  EOQ
}

trigger "query" "detect_and_correct_rds_db_instance_if_deletion_protection_disabled" {
  title         = "Detect & correct RDS DB instancesif deletion protection disabled"
  description   = "Detects RDS DB instances if deletion protection disabled and runs your chosen action."
  // documentation = file("./rds/docs/detect_and_correct_rds_db_instance_if_deletion_protection_disabled_trigger.md")
  tags          = merge(local.rds_common_tags, { class = "unused" })

  enabled  = var.rds_db_instance_if_deletion_protection_disabled_trigger_enabled
  schedule = var.rds_db_instance_if_deletion_protection_disabled_trigger_schedule
  database = var.database
  sql      = local.rds_db_instance_if_deletion_protection_disabled_query

  capture "insert" {
    pipeline = pipeline.correct_rds_db_instance_if_deletion_protection_disabled
    args = {
...
pipeline "detect_and_correct_rds_db_instance_if_deletion_protection_disabled" {
  ...
  param "default_action" {
    ...
    default     = var.rds_db_instance_if_deletion_protection_disabled_default_action
  }

  param "enabled_actions" {
    type        = list(string)
    description = local.description_enabled_actions
    default     = var.rds_db_instance_if_deletion_protection_disabled_enabled_actions
  }
...
pipeline "correct_rds_db_instance_if_deletion_protection_disabled" {
  ...
  // documentation = file("./rds/docs/correct_rds_db_instance_if_deletion_protection_disabled.md")
  tags          = merge(local.rds_common_tags, { class = "unused" })
  ...
  step "message" "notify_detection_count" {
    ...
    text     = "Detected ${length(param.items)} RDS DB instance if deletion protection disabled."
  }
  ...
  step "pipeline" "correct_item" {
    ...
    pipeline        = pipeline.correct_one_rds_db_instance_if_deletion_protection_disabled
    args = {
...
pipeline "correct_one_rds_db_instance_if_deletion_protection_disabled" {
  ...
  // // documentation = file("./rds/docs/correct_one_rds_db_instance_if_deletion_protection_disabled.md")
  tags          = merge(local.rds_common_tags, { class = "unused" })
  ...
  param "deletion_protection" {
    type        = bool
    description = "Enables the deletion protection property of a DB instance"
  }
  ...
  step "pipeline" "respond" {
    ...
    args = {
      ...
      notification_level = param.notification_level
      approvers          = param.approvers
      detect_msg         = "Detected RDS DB instance with deletion protection disabled ${param.title}."
      default_action     = param.default_action
      enabled_actions    = param.enabled_actions
      actions = {
        "skip" = {
          ...
          pipeline_args = {
            ...
            text     = "Skipped RDS DB instance ${param.title} with deletion protection disabled."
          }
          success_msg = ""
          error_msg   = ""
        },
